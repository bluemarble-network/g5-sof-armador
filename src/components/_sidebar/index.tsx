  ...
  async function getMenuData () {
    ...
    try {
      ...
      for (const index in menus) {
        ...
        let filteredSubs:any = subs.filter(sub => (sub.display_name.split('/')[0] === item.display_name))
        filteredSubs = filteredSubs.map(item => ({ to: item.url, title: item.display_name.split('/')[1] }))
                        ...
                        <List sx={{ marginTop: 10 }}>
                            {
                                menuData.map((item, index) => {
                                  ...
                                  return (
                                        <SideMenuItem setDrawerOpen={() => {}} key={index} title={item.display_name} to={item.url} icon={item.icon} />
                                  )
                                })
                            }
                            {
