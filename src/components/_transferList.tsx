...
function customList (items, checked, setChecked, getLabel) {
  return (
...
interface ITransferList {
  leftList: any[]
  rightList: any[]
  setLeftList(value: any): void
  setRightList(value: any): void
  getLabel(value: any): string
}

export function TransferList ({ leftList, rightList, setRightList, setLeftList, getLabel }: ITransferList) {
  ...
  return (
        <Box component="div" sx={{ display: 'grid', gap: 1, marginTop: 4, width: '100%', gridTemplateColumns: '2fr 1fr 2fr', justifyContent: 'center', alignItems: 'center' }}>
            {customList(leftList, checked, setChecked, getLabel)}
            ...
            {customList(rightList, checked, setChecked, getLabel)}
        </Box>
  )
}
