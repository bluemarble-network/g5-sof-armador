function formatToMoney (value:number, symbolBeforeDot?: string, symbolAfterDot?: string): string {
  if (!symbolBeforeDot) { symbolBeforeDot = '.' }
  if (!symbolAfterDot) { symbolAfterDot = ',' }

  let toString = `${value}`
  if (!toString.includes('.') && !toString.includes(',')) toString = `${toString}00`
  ...
  if (valueAfterDot && valueAfterDot.length < 2) { valueAfterDot = `${valueAfterDot}0` }
