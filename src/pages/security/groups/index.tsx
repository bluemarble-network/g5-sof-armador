import { Box, Checkbox, CircularProgress, Collapse, CssBaseline, IconButton, List, ListItem, Menu, MenuItem, Paper, Tooltip, Typography } from '@material-ui/core'
import { Form } from '@unform/web'
import Head from 'next/head'
import { useContext, useEffect, useRef, useState } from 'react'
import { MdAdd, MdChevronRight, MdDelete, MdEdit, MdExpandLess, MdExpandMore, MdMoreVert, MdRemoveRedEye } from 'react-icons/md'
  ...
  return (
        ...
        <CssBaseline/>
           ...
           >
                         ...
                         return (
                                ...
                                >
                                    <Td sx={{ position: 'relative', overflow: 'hidden' }}>
                                        <Checkbox checked={selectItem?.id === item.id} onClick={() => setSelectItem(item)} />
                                        {item.name}
                                    </Td>
                                </Tr>
                         )
            ...
            {
              selectItem &&
              <Sidebar open={sidebarOpen} setOpen={setSidebarOpen} group={selectItem} />
            }
        </Box>
  )
}
...
function Sidebar ({ open, setOpen, group }: ISideBar) {
  ...
  async function getApps () {
    try {
      const { data } = await api.get(`/applications/routes/group?group_id=${group?.id}`)
      setUsedApps(data)
  ...
  function hasRoute (route: iRoutes): any {
    let result: any = { canDelete: false, canInsert: false, canView: false, canUpdate: false }
  ...
  async function handleUpdateAccess (route, type) {
    ...
    const [storagePermisson] = usedApps.filter(item => (item.route_id === route.id))
    ...
    try {
      const { data }: { data: any } = await api.put(`/applications/routes/group?group_id=${group?.id}`, body)
      const filteredApps = usedApps.slice(0).filter(item => (item.id !== data.id))
  ...
  async function handleAddAll (application: iApps) {
    ...
    setAppLoading({ id })
    try {
      const { data } = await api.put(`/applications?id=${application_id}&group=${group?.id}&action=add`)
      createAlert('Acesso concedido com sucesso!', 'success')

      const filteredApps = usedApps.map(app => {
        if (app.routes.application_id === application_id && app.group_id === group?.id) {
          app.canView = true
          app.canUpdate = true
          app.canDelete = true
          app.canInsert = true
  ...
  async function removeAllAccess (application: iApps) {
    ...
    try {
      await api.put(`/applications?id=${application_id}&group=${group.id}&action=remove`)
      const filteredApps = usedApps.map(app => {
        if (app.routes.application_id === application_id && app.group_id === group.id) {
          app.canView = false
          app.canUpdate = false
        ...
        <>
            ...
            <Modal open={!!selectedRoute} onClose={() => setSelectedRoute(null)}>
                <Box sx={{ px: 2, py: 3, width: '100vw', maxWidth: 400 }}>
                    <Typography variant="h6" fontWeight="bold">Detalhes da rota</Typography>
                    <Box sx={{ py: 1 }} />
                    <Box sx={styles}>
                    ...
                    <Box sx={styles}>
                        <Typography variant="body1" fontWeight="bold" >Rota</Typography>
                        <Typography variant="body1" >{selectedRoute?.url}</Typography>
                    </Box>
                    <Box sx={{ ...styles, display: 'block', borderBottom: 0 }}>
                        <Typography variant="body1" fontWeight="bold" >Descrição</Typography>
                        <Box sx={{ py: 1 }} />
...
function AppItem ({ item, hasRoute, update, loading, handleRemoveAll, handleAddAll, setSelectedRoute }: AppItemProps) {
  ...
  return (
        <>
                    ...
                    item.routes.map((item, index) => {
                      ...
                      return (
                            <ListItem
                                ...
                                disablePadding
                            >
                                <Typography onClick={() => setSelectedRoute(item)} variant="caption" sx={{ textDecoration: 'underline', cursor: 'pointer' }} >{item.name}</Typography>
                                <Box sx={{ display: 'flex' }}>
                                    <IconButton onClick={() => update(item, 'canDelete')} sx={{ color: 'success.main' }} >
                                        <MdDelete size={20} color={canDelete ? 'inherit' : 'GrayText'} />
                                    </IconButton>
                                    <IconButton onClick={() => update(item, 'canInsert')} sx={{ color: 'success.main' }}>
                                        <MdAdd size={20} color={ canInsert ? 'inherit' : 'GrayText'} />
                                    </IconButton>
                                    <IconButton onClick={() => update(item, 'canUpdate')} sx={{ color: 'success.main' }}>
                                        <MdEdit size={20} color={ canUpdate ? 'inherit' : 'GrayText'} />
                                    </IconButton>
                                    <IconButton onClick={() => update(item, 'canView')} sx={{ color: 'success.main' }}>
                                        <MdRemoveRedEye size={20} color={ canView ? 'inherit' : 'GrayText'}/>
                                    </IconButton>
                                </Box>
                            </ListItem>
                      )
            ...
            >
                <MenuItem onClick={() => { handleAddAll(item); handleClose() }}>Conceder acesso a todas as rotas</MenuItem>
                <MenuItem onClick={() => { handleRemoveAll(item); handleClose() }}>Remover acesso a todas as rotas</MenuItem>
            </Menu>
        </>
  )
}
...
function FormGroups ({ selectItem }: { selectItem?: IGroup }) {
  ...
  async function handleSubmit () {
    ...
    try {
      setLoading(true)
      await api.put(`/groups/application?group_id=${selectItem?.id}`, { add: idToAdd, remove: idToremove })
      setLoading(false)
  ...
  useEffect(() => {
    (async function () {
      try {
        const { data: groupApplications } : { data: any[] } = await api.get(`/groups/application?group_id=${selectItem?.id}`)
        const { data: applications } : { data: any[] } = await api.get('/applications')

        setRightList(groupApplications)

        const groupAppId = groupApplications.map(item => item.id)
        const filteredApps = applications.filter(item => !groupAppId.includes(item.id))
  ...
  return (
        ...
        >
            ...
            <TransferList leftList={leftList} rightList={rightList} setRightList={setRightList} setLeftList={setLeftList} getLabel={(item: any) => item.displayName}/>
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'flex-end', paddingBottom: 3 }}>
...
function FormPermissions ({ selectItem }: { selectItem: IGroup }) {
  ...
  async function handleUpdateAccess (item: any) {
    try {
      await api.put(`/groups/permissions?id=${item.id}`, { data: { canInsert: item.canInsert, canUpdate: item.canUpdate, canDelete: item.canDelete, canView: item.canView } })
  ...
  async function getData () {
    setLoading(true)
    try {
      const { data }: { data: any[] } = await api.get(`/groups/permissions?group_id=${selectItem.id}`)
      setData(data.slice(0, 6))
      setLoading(false)
    } catch (error) {
      setLoading(false)
            ...
            >
                {data.map((item, index) => {
                  return (
                        <Tr key={index}>
                            <Td>
                                {item.applications.displayName}
                            </Td>
                            <Td>
                                {item.applications.url}
                            </Td>
                            <Td sx={{ width: 200 }}>
