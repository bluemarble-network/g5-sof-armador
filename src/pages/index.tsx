...
function Login() {
  ...
  return (
    <Box
      component='main'
      sx={{ paddingTop: 10, maxWidth: 350, margin: 'auto' }}
    >
      <CssBaseline />
      <Paper
        sx={{
...
export async function getServerSideProps({ req }) {
  ...
  if (
    cookie.includes('next-token') ||
    cookie.includes('next-refresh-token') ||
    cookie.includes('next-remember-me-token')
  ) {
    return {
      redirect: {
        destination: '/dashboard-vessel?topBar=hidden'
      }
    }
  } else {
    return { props: {} }
